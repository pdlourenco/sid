...
**Origin:** the "Revised Roadmap" and "Timeline" sections of `docs/roadmap.md`,
reproduced **verbatim** below, extracted when the roadmaps were restructured
along the contract/implementation layers (issue #195, ADR-0006). This is the
phase log only — it is not a copy of the whole former file. That file's other
sections were relocated (naming convention and catalogue stayed in
[`docs/roadmap.md`](../roadmap.md); Octave compatibility rules moved to
[`matlab/CONTRIBUTING.md`](../../matlab/CONTRIBUTING.md)) or deliberately
deleted ("Result Struct", which duplicated the spec; "Package Structure" and
"Usage", both stale).
