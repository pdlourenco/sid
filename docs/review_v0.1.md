...
- Both test suites pass at HEAD: Python 329/329; MATLAB/Octave suite not run here (no Octave in this environment) but is exercised in CI.
...
**Residual limitations of this review:** `spec/cosmic/online_recursion.md` was only skimmed (feature deferred to v2); EXAMPLES.md §3's per-example parameter values were checked for presence/structure, not re-derived line-by-line; the MATLAB test suite was not executed in this environment (CI runs it).
