...
2. *Inner loop:* run the alternating state–COSMIC loop at the fixed current $\mu$ until the relative cost change falls below $\varepsilon_J$ **or** the per-stage inner cap `MaxIter` is reached. Report $J^*(\mu)$ and $(X^*, C^*)_\mu$ as the **best (lowest-cost) iterate observed during the stage** — not necessarily the last, since for $\mu > 0$ the inner iteration is a homotopy fixed-point (not the monotone descent of Section 4.2), so a capped non-converged stage could otherwise report a worse $J^*$ than it visited.
3. Propose $\mu' = \mu / 2$; run the inner loop at $\mu'$, yielding $J^*(\mu')$.
4. **Accept/reject:** if $J^*(\mu') \leq J^*(\mu)$, **accept** ($\mu \leftarrow \mu'$, keep $(X^*, C^*)_{\mu'}$) and repeat from step 3. If $J^*(\mu') > J^*(\mu)$, **reject** — restore $(X^*, C^*)_\mu$ and stop reducing $\mu$.
5. *Final $\mu = 0$ refinement (both exits).* On leaving the outer loop — whether by **reject** (step 4) or by $\mu < \varepsilon_\mu$ (default $\varepsilon_\mu = 10^{-6}$) — run one inner loop at $\mu = 0$ from the current best iterate and **keep its result only if it lowers $J^*$** (a guarded pass; costless in the worst case). Return that iterate. This guarantees the returned iterate is a fixed point of the reported ($\mu = 0$) alternation rather than one smoothed under $\tilde{A} \neq A$. `MaxIter` is a **per-stage** inner cap, so the worst-case total inner-iteration count is $\texttt{MaxIter} \times (\lceil \log_2(1/\varepsilon_\mu) \rceil + 2)$. See SPEC §8.12.4 for the normative budget and defaults.
