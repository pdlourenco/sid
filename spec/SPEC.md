...
- §8.4.2 L-curve auto-λ corner selection — `unit(M)` `test_sidLTVdisc.m` (Test 21), `unit(Py)` `test_ltv_disc.py` (`test_auto_lambda_selects_interior_corner`): asserts the selected λ is an **interior** grid point (a real curvature corner, not a degenerate endpoint) and recovers the system (#120). **`none` for the §8.3.4 ill-conditioning warning (`sid:singularLbd`)** — a defensive forward-pass diagnostic not reachable from normal public-API inputs; exercising it needs a targeted internal-solver fixture (#120).
